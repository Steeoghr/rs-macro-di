...
use quote::quote;
use syn::{parse_macro_input, AttributeArgs, FnArg, ItemFn, Lit, Meta, MetaNameValue, NestedMeta, PatType};
...
fn generate_route(method: &str, args: TokenStream, input: TokenStream, has_body: bool) -> TokenStream {
    ...
    // Estrai i parametri dal metodo
    let method_params: Vec<_> = input.sig.inputs.iter().map(|arg| {
